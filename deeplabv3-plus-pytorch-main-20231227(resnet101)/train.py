...
import numpy as np
import torch             #PyTorch框架，用于构建和训练神经网络。
import torch.backends.cudnn as cudnn      #用于配置cuDNN的选项。
import torch.distributed as dist
import torch.optim as optim          #包含各种优化算法。
from torch.utils.data import DataLoader       #用于数据加载和处理的工具。

from nets.deeplabv3_plus import DeepLab        #用于数据加载和处理的工具。
from nets.deeplabv3_training import (get_lr_scheduler, set_optimizer_lr,       #导入DeepLabv3+模型的训练相关函数。
                                     weights_init)
...
'''
if __name__ == "__main__":             #Python的入口点，确保下面的代码只在作为主程序运行时执行。
    #---------------------------------#
    #   Cuda    是否使用Cuda
    #           没有GPU可以设置成False
    #---------------------------------#
    ...
    #   xception
    #---------------------------------#
    backbone        = "MobileNetV2"      #设置模型的主干网络为MobileNetV2。
    #----------------------------------------------------------------------------------------------------------------------------#
    ...
    '''预训练权重的路径。'''
    model_path      = "./model_path/deeplab_mobilenetv2.pth"        #权重文件在logs/model_path里。
    #---------------------------------------------------------#
